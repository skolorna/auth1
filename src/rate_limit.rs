...
pub trait RateLimit {
    ...
    fn remaining_requests(&self, remote_ip: &IpAddr, redis: &mut RedisConn) -> Result<u64>;
}
...
impl SlidingWindow {
    ...
    fn key(&self, remote_ip: &IpAddr, window_id: u64) -> String {
        format!("{}:{}:{}", self.key_prefix, remote_ip, window_id)
    }
    ...
    fn record(&self, remote_ip: &IpAddr, conn: &mut RedisConn) -> Result<u64> {
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        let current_window = self.window_id(now);
        let current_key = self.key(remote_ip, self.window_id(now));
        let previous_key = self.key(remote_ip, current_window - 1);
...
impl RateLimit for SlidingWindow {
    fn remaining_requests(&self, remote_ip: &IpAddr, redis: &mut RedisConn) -> Result<u64> {
        let count = self.record(remote_ip, redis)?;
